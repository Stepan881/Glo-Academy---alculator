...
function renderTextContent(total, site, maxDay, minDay){
    ...
    Сделаем ${site}${adaptCheckBox.checked ?
...
function priceCalculation(elem = {}) {
    ...
    for (const item of formCalculate.elements) {
        if (item.name === 'whichSite' && item.checked) {            
            ...
            overPrecent = overDay * DATA.deadlinePercent[index] / 100;
...
function handlerCallBachForm (event) {
...
priceCalculation();
formCalculate.addEventListener('change', handlerCallBachForm);